...
console.log(bowl); //변수 정의되지않음 undefined(error)
var bowl; //빈 그릇
console.log(bowl); //변수 정의되지않음 undefined
bowl = '밥';// ''(큰따옴표, 작은따옴표 모두 가능)문자데이터를 변수에 대입
console.log(bowl);
bowl = '콩';// '밥' 기존데이터 제거하고 '콩' 새로운 데이터대입
console.log(bowl); //console객체에 메서드log로 bowl변수를 실행
...
var b;
var c;
console.log(a, b, c); //값정의 안됨*3 undefined
...
const y = 5; //상수 5생성, 대입 동시 수행
console.log(x, y);
x = 0; //x 값 변경(기존값 제거)
// y = 7; //상수를 변경하려고하면 에러발생

// 웹사이트 가입 회원정보 변수
// 이름, 나이, 사는 곳 데이터를 담는 변수 생성하기
let user_name = '홍길동';
let user_age = 20;
let user_address = '인천 부평구';
...
console.log(`${user_name}은 ${user_age}살이고 ${user_address}에 삽니다.`);//템플릿문자열 활용 출력
// + 연결연산자, 더하기연산자
// 홍길동은 내년에 21살이 됩니다.
// user_name은 내년에 user_age+1살이 됩니다.
...
// num변수의 데이터종류는 ?
console.log(`num변수의 데이터종류는 ${typeof num}`); // typeof -> 데이터 종류 확인용
console.log(`num변수의 데이터종류는 ${typeof(num)}`); // typeof는 함수종류이기때문에, 메서드와 같은 방식으로 적어도됨.