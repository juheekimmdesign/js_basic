...
const gnbClone = gnbUl.cloneNode(true); //복제(Ctrl+C와 유사)
console.log(mNav, gnbUl, gnbClone);

mNav.appendChild(gnbClone); //자식에 붙여넣기


// 데스크톱 메뉴(gnb) 복제해서 모바일 메뉴 위치(login_cart)에 붙여넣기
const mLoginCart = mBg.querySelector('.login_cart')
const dLoginCart = document.querySelectorAll('.right_menu .login_cart a')//복제 전 선택

// 특정 대상 복제 객체.cloneNode() 내장함수 사용
const dloginClone = dLoginCart[0].cloneNode(true);//복제(Ctrl+C와 유사)
const dCartClone = dLoginCart[1].cloneNode(true);//복제(Ctrl+C와 유사)

mLoginCart.appendChild(dloginClone); //자식에 붙여넣기
mLoginCart.appendChild(dCartClone); //자식에 붙여넣기

console.log(mLoginCart,dLoginCart[0],dLoginCart[1],dloginClone,dCartClone);
