...
// log 실행하고 싶은 함수 명령어 -> (괄호) 안에 있는 것을 실행한다.
...
let x;
const y = '20';
console.log(x, y); // x 값이 정의 안됨(undefined), '20'
console.log(y+b); //201
// y 초기화하고싶다 -> 불가능 -> const 상수
x = '10'; // 정의안됨 -> 문자 10 대입
console.log(x+y); //1020
x = null; // x변수의 값을 초기화한다. (null == 0)
console.log(x+y); // null20 -> y가 문자데이터이기 때문에 null도 문자로 출력됨
console.log(x+a); // 1 -> 0과 숫자1을 더함

console.log('-----------------------------------------')

// log(), typeof()
// prompt() -> 질문, 답변창을 띄우는 함수(문자열로 인식)
// Number() -> 괄호안의 데이터를 숫자로 변환하는 함수
// 유치원생 대상 두 수를 더하는 문제 프로그래밍
const num1 = Number(prompt('첫번째 수를 입력하세요')); //-> 객체.메서드(메서드());
// 1.const num 상수 생성
