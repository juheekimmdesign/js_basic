...
window.addEventListener('scroll', function(){
    ...
    // 100 위치 만큼 내리면 '위로' 나타나기
    if(window.scrollY >= 100) fixedTop.style.display ='block';
    // 4. 위로 이동하고 나면 다시 숨겨지기
    else if(window.scrollY < 100) fixedTop.style.display ='none';

    // =======================================================header
    // 1. 특정위치까지 스크롤을 내리면 header 고정시키고 디자인변경하기
    // 2. 스크롤을 올리면 header 디자인 복구, 고정 해제하기
    if(window.scrollY >= 200){
        console.log('200보다 큰 위치');
        // header.style.position = 'fixed'; // 고정(body 기준으로 기존 디자인(위치) 해제)
        // header.style.position = 'sticky'; // 고정(태그 구조상 부모 기준 고정(디자인 2중으로 안해도됨) -> 부모가 짧으면 적용 잘 안됨)
        // header.style.top = '0';
