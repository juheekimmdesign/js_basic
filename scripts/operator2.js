// operator2.js
// 증감연산자 ++, --
let x = 5; // x변수생성 -> 숫자5를 x로 대입
// let y = ++x; // y변수생성 -> 1증가한 x를 대입(6,6)
let y = x++ // y변수 생성 -> x를 대입하고 나중에 x를 1증가(6,5)
console.log(x, y);
...
// a = 1;
// b = y;
// console.log(`a=${a}, b=${b}`); // a=1, b=5 템플릿문자열

let a;
let b;
let c;
let d;
// 변수만 생성하고 값이 정의 안된 상태(undefined)
a = 1;
b = 2;
c = 3;
d = 4;
...
// num++ 증가전 값이 plus 인식 -> 전위 연산 ++num
// let num = Number(prompt('구입 수량을 입력하세요.'));
let num = Number('1'); // (위) 아래부터 프롬프트 확인을 위해 수정해둠
let plus = ++num;
...
// 복합대인연산자는 새로운 값이 아닌 기존값을 활용해서 추가대입
// 키 140 -> 145
let height = 140;
...
let wallet = 10000; // (위) 아래부터 프롬프트 확인을 위해 수정해둠
...
// 결과형태) 편의점 총 구입비는 00000원이고 남은 돈은 0000원입니다.
wallet = 10000;
let kimbab = 2000;
let icecream = 3000;
let snack = 4000;
...
result = `현재 남은돈은 ${wallet}원`;
console.log(result);
result = `총 구입비는 ${total.toLocaleString('ko-kr')}원, 남은 돈은 ${wallet.toLocaleString('ko-kr')}원 입니다.`;
console.log(result);