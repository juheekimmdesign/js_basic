...
console.log(color);
console.log(`내가 좋아하는 색상은 ${color[3]}, ${color[4]}입니다.`);
...
// 날씨 상태를 저장한 배열
const weather = ['맑음', '비', '흐림', '눈'];

// 함수 실행시마다 다른 데이터를 전달하는 매개변수
weatherFunc(weather[0], weather[1]); //호출
weatherFunc(weather[2], weather[3]);
weatherFunc(weather[3], weather[0]);

function weatherFunc(today, tomorrow){ //준비 매개함수 -> 매개생성, 값대입받음, 내부함수에 값전달
...
let cgvSeat = new Array(2); // 빈좌석 2개 준비
// cgvSeat[0] = prompt('선택좌석은?'); -> prompt활용시
cgvSeat[0] = 'K1'; 
cgvSeat[1] = 'K2';

// 함수호출
cgvFunc(cgvSeat[0], cgvSeat[1], price*cgvSeat.length); // 배열 수에 따라 상태를 인식할 수있도록 length 사용
...
function dan99(num){//num 매개변수 생성(호출시 대입받는 값을 받는 위치)
    let dan ='';
    dan += `${num}X1=${num*1}`; //num 호출시 대입받은 값을 최종 전달받는 위치
    dan += `${num}X2=${num*2}`;
    dan += `${num}X3=${num*3}`;
    dan += `${num}X4=${num*4}`;
    dan += `${num}X5=${num*5}`;
